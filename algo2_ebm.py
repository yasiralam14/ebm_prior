...
class OptimusDecoderWrapper(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, past=None):
        # When past (latent z) is provided, Optimus injects it as 1 virtual
        # KV token per layer → effective key length = seq_len + 1.
        # We must extend the attention mask by 1 on the LEFT (the latent slot)
        # so it matches the expanded key dimension inside _attn.
...
class SmallRandomDecoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, past=None):
        ...
        if past is not None:
            ...
            # Element-wise addition of token embedding, pos embedding, and projected latent
...
def sample_langevin_prior(z, ebm, K_0=60, a_0=0.4):
    ...
    for i in range(K_0):
        # Calculate EBM Energy
        en = ebm(z).squeeze(-1)
        z_grad = torch.autograd.grad(en.sum(), z)[0]
        
        # Calculate gradient of standard normal prior: grad(0.5 * ||z||^2) = z
        grad_prior = z.clone()
...
def sample_langevin_posterior(z, dec_input_ids, target_ids, dec_attention_mask, dec_word_mask, G, E, K_1=40, a_1=0.1, llhd_weight=1.0):
    ...
    for i in range(K_1):
        # 1. EBM Energy Gradient
        en = E(z).squeeze(-1)
        grad_e = torch.autograd.grad(en.sum(), z)[0]
        
        # 2. Decoder Likelihood (Reconstruction Loss NLL)
        out = G(
        ...
        # Log Iteration Metrics tracking energy & recon
        with torch.no_grad():
            mcmc_log_table.append({
                "step": i,
                "total_energy": ((llhd_weight * total_reco_loss_sum) + en + 0.5 * (z**2).sum(-1)).mean().item(),
                "ebm_energy": en.mean().item(),
                "recon_loss": sentence_recon_loss.item(), # Logging mean NLL per sentence
