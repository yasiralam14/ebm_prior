...
def train_unimodal_ebm(encoder, decoder, train_loader, epochs=10, latent_dim=768, lr=3e-4, beta=1.0, max_norm=1.0, device='cuda'):
    ...
    """
    # 1. Initialize wandb (You should call wandb.init() somewhere before this function!)
    # if wandb.run is None:
    #     wandb.init(project="ebm_prior_training")
    
    # 2. Initialize the combined Model and Sampler
    ...
    # 3. Initialize separate Optimizers for alternating training
    # VAE optimizer includes encoder and decoder parameters
    optimizer_vae = torch.optim.Adam([
        {'params': model.encoder.parameters()},
        {'params': model.decoder.parameters()}
    ], lr=lr)
    
    # EBM optimizer only includes the prior (EBM) parameters
    # Set to 10x the VAE learning rate so the EBM learns fast enough to carve deep craters.
    ...
    for epoch in range(epochs):
        ...
        # We will log metrics directly every 100 batches instead of averaging
        
        # We will keep the MCMC table from the final batch of the epoch to log its dynamic behavior
        last_mcmc_log_table = None
        
        progress_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}")
        for batch_idx, batch in enumerate(progress_bar):
            # Calculate dynamic beta (Cyclic Annealing)
            ...
            # Extract input components robustly
            ...
            # Core parameter update algorithm
            ...
            # Log current batch metrics every 100 batches to avoid smearing the data
        ...
        # 6. Wandb Logging
        # Translate the python generic dict table into a wandb.Table
        if last_mcmc_log_table is not None:
            ...
            for row in last_mcmc_log_table:
                # Add data sequentially corresponding to col order
                wandb_table.add_data(*[row[col] for col in table_columns])
            
            # Log the MCMC table at the end of the epoch
...
print(dec_ids.shape, dec_mask.shape, enc_ids.shape, enc_mask.shape)  # all [B, 50]
print("dec pad id:", dec_tok.pad_token_id, "enc pad id:", enc_tok.pad_token_id)


import os
wandb.login(key=os.environ.get("WANDB_API_KEY"))  # Set WANDB_API_KEY in your environment
